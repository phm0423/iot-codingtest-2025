...
- VS Code 대체 에디터
    1. VSCodium - http://vscodium.com/
        - VS Code와 동일한 기능 제공
    2. Cursor AI - http://cursor.com
        - 바이브 코딩용
...
- 깃헙주소 - https://github.com/dremdeveloper/codingtest_python
...
### 코딩테스트 사이트
- 프로그래머스 - http://school.programmers.co.kr
...
- 입력값 파악: 입력값의 크기에 따라 `시간복잡도`가 결정됨
- 핵심키워드 파악: 알고리즘, 자료구조 선택

#### 프로그래머스
- 카카오가 가장 많이 사요하고 있는 코테사이트
...
|순번|시간복잡도|최대연산 횟수|비교|
|--:|:--|--:|:--|
|1|$O(\log n)$|1000000000|단일 for문 중간 연산을 빠지는 경우가 매우 많은 경우(ex: 3, 6, 9)|
|2|$O(n)$|20000000|for문 사용 / 반복문 하나 사용|
|3|$O(n\log n)$|1000000|2중 for문에서 반복문하나가 log n 횟수로 연산|
|4|$O(n^2)$|5000|2중 for문|
|5|$O(n^3)$|300|3중 for문|
...
- 박테리아 계산도 16개를 매년 하나씩 줄어들면 16년이 걸릴것으로 예상(n)
